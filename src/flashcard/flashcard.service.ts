...
import { CreateFlashcardDto, ReviewFlashcardDto } from './dto/create-flashcard.dto';
...
export class FlashcardService {
  ...
  async create(body: CreateFlashcardDto) {
    try {
      const course = await this.courseModel.findById(body.course);
      if (!course) return { status: 404, error: 'Course not found' };

      const newItems = (body.cards || []).map((c: any) => ({
        front: c.front || '',
        back: c.back || '',
        tags: c.tags || [],
        creator: new mongoose.Types.ObjectId(body.creator),
  ...
  async getById(courseId: string, userId: string) {
    try {
      const course = await this.courseModel.findById(courseId).select('flashcards title');
      if (!course) return { status: 404, error: 'Course not found' };
  ...
  async update(courseId: string, userId: string, body: any) {
    try {
      ...
      if (body.cards && Array.isArray(body.cards)) {
        ...
        course.flashcards = body.cards.map((c: any) => ({
          front: c.front || '',
          back: c.back || '',
          tags: c.tags || [],
          creator: new mongoose.Types.ObjectId(c.creator || userId),
          easeFactor: typeof c.easeFactor === 'number' ? c.easeFactor : 2.5,
          interval: typeof c.interval === 'number' ? c.interval : 0,
          dueDate: c.dueDate || null,
  ...
  async reviewCard(courseId: string, userId: string, cardIndex: number, rating: string) {
    try {
      const course = await this.courseModel.findById(courseId);
      if (!course) return { status: 404, error: 'Course not found' };
      ...
      if (q < 3) {
        ...
        if (rating === 'Easy') card.easeFactor = card.easeFactor + 0.15;
        if (rating === 'Good') card.easeFactor = card.easeFactor + 0.05;
      }
  ...
  async delete(courseId: string, userId: string, cardIndexOrId: any) {
    try {
      const course = await this.courseModel.findById(courseId);
      if (!course) return { status: 404, error: 'Course not found' };
      ...
      if (typeof cardIndexOrId === 'number') {
        ...
        course.flashcards = course.flashcards.filter((c: any) => {
  ...
  async getDueForUser(courseId: string, userId: string) {
    try {
      const course = await this.courseModel.findById(courseId).select('flashcards');
      if (!course) return { status: 404, error: 'Course not found' };
      const now = new Date();
      const due = (course.flashcards || []).filter((c: any) => {
  ...
  async generateFromPDF(courseId: string, userId: string, topic?: string) {
    try {
      ...
      const genResult = await this.groqService.generateFlashcardsFromPDF(
        course.pdfContent,
        course.title,
      );
      ...
      const normalizeQA = (item: any) => {
        const pick = (keys: string[]) => {
          for (const k of keys) {
            if (item[k] && typeof item[k] === 'string' && item[k].trim().length > 0) return item[k].trim();
          }
          return '';
        };
